module.exports={
  ...
  extraMetadata:{main:'desktop/main.cjs',version:'1.0.0-preview.2'},
  asar:true,npmRebuild:false,afterPack:'desktop/after-pack.cjs',
  mac:{target:[{target:'dmg',arch:['arm64','x64']}],category:'public.app-category.healthcare-fitness',icon:'desktop/PKDCompass.icns',hardenedRuntime:true,gatekeeperAssess:false,identity:process.env.CSC_NAME||'-',notarize:!!process.env.APPLE_API_KEY||!!process.env.APPLE_ID},
  dmg:{title:'PKD Compass Installer',icon:'desktop/PKDCompass.icns',contents:[{x:150,y:160,type:'file'},{x:430,y:160,type:'link',path:'/Applications'}]},
